...
- [x] Initialize Expo project in `mobile/`: `npx create-expo-app mobile --template blank-typescript`
- [x] Install navigation: `expo-router` or `@react-navigation/native`
- [x] Install location: `expo-location`, `expo-task-manager`
- [x] Install notifications: `expo-notifications`
- [x] Create `mobile/services/api.ts` — typed API client pointing to FastAPI backend; attach JWT to all requests
- [x] Create `mobile/services/auth.ts` — `register()`, `sendOtp()`, `verifyOtp()`, store JWT in `SecureStore`
- [x] Implement Onboarding screen: phone number input → OTP → registration form (name, city, dark store zone, avg earnings, UPI ID)
- [x] Implement Home screen: display active policy tier + weekly premium, live DCI score for worker's hex (color-coded), active coverage cap
- [x] Implement Payout History screen: list of claims with amount, date, resolution path, status
- [x] Implement background location task using `expo-task-manager`: ping every 15 minutes, include GPS coords, accuracy, network signal, mock location detection; POST to `/location-pings`
- [x] Implement FCM push notification handler: display in-app notification on payout, alert, or verification request
- [x] Implement Tier Upgrade prompt screen: shown when Sunday forecast predicts DCI > 0.75 for worker's hex
